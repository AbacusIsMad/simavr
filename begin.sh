...
AVR_PKGS = -I$EXTRACT_PATH -I$EXTRACT_PATH/avr/include -I$EXTRACT_PATH/arduino_lib/include -L$EXTRACT_PATH/avr/lib $EXTRACT_PATH/arduino_lib/lib/core.a

SRC = \$(wildcard *.c) \$(wildcard *.S)

#main.o: main.c
#	\$(AVR_ROOT)/\$(AVR_CC) -nostdinc -Os \$(AVR_CFLAGS) \$(AVR_PKGS) $< -o $@

all: \$(SRC)
	\$(AVR_ROOT)/\$(AVR_CC) -nostdinc $^ -o main.elf -Os \$(AVR_CFLAGS) \$(AVR_PKGS)
